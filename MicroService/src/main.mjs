...
export async function CreateService({
  ...
  try {
    fs.watch(STREAMS_DIR, { persistent: true }, () => {
      clearTimeout(fs._reloadTimer);
      fs._reloadTimer = setTimeout(reloadStreamsImpl, 150);
      fs._reloadTimer?.unref?.();
    });
  } catch {
  ...
  const INITIAL_BACKFILL_MS = Number(
    config.mongo.timeSeries?.initialBackfillMs ?? 10_000
  );

  // ---- Processors registry ----
  const processors = new Map(); // id -> async (event, ctx) => void
  function onStream(id, handler) {
    processors.set(id, handler);
  ...
  async function routeDoc(doc, srcTag) {
    ...
    const method = doc?.route?.method || doc?.route?.Method || "GET";
    const pathStr =
    ...
    const handler = processors.get(streamId);
    if (handler) {
      await handler(doc, {
  ...
  /* -------------------- Poller core -------------------- */
  const pollers = new Map(); // coll -> interval handle
  ...
  async function startPoller(coll) {
    ...
    const h = setInterval(() => {
      pollOnce(coll).catch((err) =>
        logger.error({ err, coll }, "ts_poller_tick_error")
      );
    }, POLL_MS);
    h.unref?.();
    pollers.set(coll, h);
  ...
  async function shutdown() {
    try {
      for (const [coll, h] of pollers) clearInterval(h);
